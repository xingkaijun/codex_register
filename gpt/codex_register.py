...
用法:
    ...
    python codex_register.py --proxy socks5://arm:40710 # 指定代理
    ...
    default = {
        "name_pool": {
            "given_names": [
                ...
                "Jacob","Gary","Nicholas","Stephen","Larry","Jonathan","Scott","Frank","Benjamin","Gregory",
                "Samuel","Alexander","Patrick","Jack","Dennis","Jerry","Tyler","Aaron","Jose","Henry","Douglas",
                "Adam","Peter","Zachary","Nathan","Walter","Kyle","Harold","Carl","Arthur","Gerald","Roger",
                "Keith","Jeremy","Terry","Lawrence","Christian","Albert","Joe","Noah","Ethan","Albert","Mason",
                "Logan","Brian","Jackson","Isaac","Caleb","Owen","Cameron","Wyatt","Jayden","Luca","Luke",
                "Natalie","Lily","Emma","Olivia","Ava","Sophia","Isabella","Mia","Charlotte","Amelia",
                "Harper","Evelyn","Abigail","Emily","Elizabeth","Sofia","Avery","Ella","Madison","Scarlett",
                "Victoria","Riley","Samantha","Grace","Chloe","Lillian","Zoe","Hannah","Aria","Aurora",
                "Camila","Georgia","Penelope","Aaliyah"," Avery","Eva","Khloe","Makyla","Rylee","Talia"
            ],
            "family_names": [
                ...
                "Gomez","Sullivan","Miller","Barrera","Mendez","Cruz","Vasquez","Henderson","Barnes","Coleman",
                "Butler","Russell","Bell","Cole","Allen","Long","Foster","Jimenez","Watson","Morris",
                "Wood","Baez","Patel","Chavez","Huynh","Schmidt","Klein","Myers","Price","Rogers",
                "Castillo","Cox","Hamilton","Graham","Fleming","Soto","Cowan","Howell","Lyons","Carpenter"
            ]
        }
    }
    try:
        if os.path.exists(CONFIG_FILE):
            with open(CONFIG_FILE, "r", encoding="utf-8") as f:
                ...
                # 合并姓名库，用户配置优先
                if "name_pool" in user_cfg:
                    pool = user_cfg["name_pool"]
                    if "given_names" in pool and isinstance(pool["given_names"], list) and len(pool["given_names"]) > 0:
                        default["name_pool"]["given_names"] = pool["given_names"]
                    if "family_names" in pool and isinstance(pool["family_names"], list) and len(pool["family_names"]) > 0:
                        default["name_pool"]["family_names"] = pool["family_names"]
    except Exception as e:
        log.warning(f"加载配置失败，使用内置默认: {e}")
    return default

# 加载配置
_CFG = _load_config()
_GIVEN_NAMES = _CFG["name_pool"]["given_names"]
_FAMILY_NAMES = _CFG["name_pool"]["family_names"]

# 邮箱后缀
EMAIL_DOMAINS = ["kjxing.ggff.net", "kjxing.de5.net"]
# Token 上传服务器
CPA_URL = "http://136.115.135.147:3001/"#http://your-server:port
MANAGEMENT_KEY = os.getenv("MANAGEMENT_KEY", "xkj903")
# MailAPI 配置（固定）
MAIL_API_URL = "https://mail.810903.xyz"
MAIL_API_AUTH = "xkj903"
...
_BROWSER_PROFILES = [
    # Edge (基于 Chromium)
    'edge99', 'edge101',
    # Chrome - 覆盖 99 到最新（逐步增加新版本）
    'chrome99', 'chrome100', 'chrome101', 'chrome104', 'chrome107', 'chrome110',
    'chrome116', 'chrome119', 'chrome120', 'chrome123', 'chrome124', 'chrome125',
    'chrome126', 'chrome127', 'chrome128', 'chrome129', 'chrome130', 'chrome131',
    'chrome133a', 'chrome136', 'chrome142',
    # Safari (macOS/iOS)
...
# Accept header 的多样性变体（模拟不同浏览器的默认值）
_ACCEPT_VARIANTS = [
    # Chrome 典型
...
# Sec-Fetch-* 头多样性（保持逻辑一致，仅随机 Site）
...
def _pick_fingerprint() -> tuple[str, dict]:
    ...
    fetch_site = random.choice(_SEC_FETCH_SITES)
    headers = {
...
def _do_one(
    ...
    for attempt in range(1, MAX_RETRY_PER_ACCOUNT + 1):
        ...
        if attempt > 1:
            # 指数退避 + 抖动，降低短时间重复请求触发风控概率
            backoff = min(retry_max_delay, retry_base_delay * (2 ** (attempt - 2)))
            sleep_s = random.uniform(backoff * 0.8, backoff * 1.2)
            log.info(f"  重试 #{attempt}... 等待 {sleep_s:.1f}s")
            time.sleep(sleep_s)
        ...
        for attempt in range(1, 4):
            try:
                ...
                if resp.status_code in (200, 201):
                    ...
                    if attempt < 3:
                        backoff = 2 ** attempt
                        log.warning(f"  ⚠️ 上传失败 (HTTP {resp.status_code})，{backoff}s 后重试...")
                        time.sleep(backoff)
                        continue
                    failed += 1
                ...
                if attempt < 3:
                    backoff = 2 ** attempt
                    log.warning(f"  ⚠️ 上传异常: {e}，{backoff}s 后重试...")
                    time.sleep(backoff)
                    continue
                failed += 1
...
def main():
    ...
    parser.add_argument("--proxy", default="", help="手动指定单一代理；留空时走代理池随机出口")
    parser.add_argument("--retry-base-delay", type=float, default=8.0, help="重试基础退避秒数（默认 8）")
    parser.add_argument("--retry-max-delay", type=float, default=60.0, help="重试最大退避秒数（默认 60）")
    ...
    # 默认走代理池；只有手动指定 --proxy 时才固定单一出口
    if args.proxy:
        proxy_pool = [args.proxy]
        log.info(f"🌐 手动代理: {args.proxy}")
    else:
        proxy_pool = load_proxy_pool()
        if proxy_pool:
            log.info(f"🌐 代理池: {len(proxy_pool)} 个可用代理")
        else:
            log.warning("⚠️ 代理池为空，将直连（极易被封）")
            # 给用户一个明确的退出机会（不自动退出，但提示严重性）
    ...
    if stats['fail'] > 0:
        ...
        for item in stats['failed_list']:
            error_counter[item['error']] += 1
        log.info("  📊 失败原因统计:")
        for err, cnt in error_counter.most_common():
            log.info(f"    - {err}: {cnt} 次")
        # 显示前 5 个失败邮箱的详情
        log.info("  📋 失败详情（前 5）:")
        for i, item in enumerate(stats['failed_list'][:5], 1):
            log.info(f"    {i}. {item['email']} - {item['error']}: {item['message'][:80]}")
            if len(stats['failed_list']) > 5 and i == 5:
                log.info(f"    ... 还有 {len(stats['failed_list']) - 5} 个未显示")
                break
    ...
    # 上传并清理
    uploaded, upload_failed = 0, 0
    if stats["ok"] > 0 and CPA_URL:
        uploaded, upload_failed = upload_and_cleanup(RESULTS_DIR)
        log.info(f"  📤 上传统计: {uploaded} 成功, {upload_failed} 失败")
